...
def random_batch(rng: jax.random.PRNGKey, batch_size: int, devices: list[jax.Device]):
    ...
    board = jax.random.randint(rngs[0], (batch_size, 5, 6), 0, 12, dtype=jnp.int32)
    board = board.astype(jnp.float32)
    current = jax.random.randint(rngs[1], (batch_size,), 0, 12, dtype=jnp.int32)
    current = current.astype(jnp.float32)
    nxt = jax.random.randint(rngs[2], (batch_size,), 0, 12, dtype=jnp.int32)
    nxt = nxt.astype(jnp.float32)
